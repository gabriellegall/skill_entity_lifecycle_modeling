...
, period_lifecycle_date_arrays AS (
    SELECT
        ...
        LIST(DATE_INFO ORDER BY DATE_INFO ASC) FILTER (WHERE DATE_INFO = FIRST_ACTIVE_DATE_INFO)                                                     AS ALL_ACQUISITION_DATE_INFO,
        LIST(DATE_INFO ORDER BY DATE_INFO ASC) FILTER (WHERE PREVIOUS_IS_ACTIVE = TRUE AND IS_ACTIVE = FALSE)                                        AS ALL_CHURN_DATE_INFO,
        LIST(DATE_INFO ORDER BY DATE_INFO ASC) FILTER (WHERE PREVIOUS_IS_ACTIVE = FALSE AND IS_ACTIVE = TRUE AND DATE_INFO > FIRST_ACTIVE_DATE_INFO) AS ALL_RESURRECTION_DATE_INFO
